...
public class GpsMsgUtilClient {
	 ...
	 * @return
	 */
	public static List<Tdata> generateMsg(byte[] b2 ,String msgid,String mobile,String sfjm,int msgserno){
		...
		if(len<1){
			...
			hexstring=GpsMsgUtil.getMsg(b1, b2);
			...
			for(int k=0;k<len;k++){
				...
				String xhs=GpsMsgUtil.getMsg(b1, t);
	...
	public static List<Tdata> generateMsg(byte[] b2 ,String msgid,String mobile,String sfjm){
		...
		String hexstring="";
		if(len<1){
			int msgserno= GpsMsgUtilClient.generateLsh();
            if(NettyConf.debug){
                Log.e("TAG","流水号："+msgserno);
            }
			//无分包时
			...
			int msgserno= GpsMsgUtilClient.generateLshPl(len);
			//按批次数取连续流水号
			for(int k=0;k<len;k++){
				GpsHeader h2=new GpsHeader();
				h2.setMsgid(msgid);
				byte[] t=new byte[0];
				if(k==(len-1)){
					//是最后一个时直接取余数
	...
	public static byte[] getMsgExtend(byte[] b3,String msgid,String ywlx,String sfjm){
		...
		m.setMsgsx(msgsx);
		m.setJpbxh(GpsMsgUtilClient.generateJpbxh());
	 ...
	 * @return
	 */
	public static synchronized int generateLshPl(int count){
		...
		int lsh= lshsp.getInt("gpslsh",0);
		if(lsh+count>=32000){
			edit.putInt("gpslsh",0);
		}else{
			edit.putInt("gpslsh",lsh+count);
		}
		edit.commit();
		return lsh;
	}
     ...
     * @return
     */
	public String getCommonRC(GpsHeader header,String jg){
		CommonR cr=GpsMsgUtil.getCommonRhs(header, jg);
		byte[] b2=cr.getCommonRBytes();
	 ...
	 * @return
	 */
	public byte[] getCommonRheaderC(GpsHeader header){
		GpsHeader h=new GpsHeader();
		h.setMsgid("0001");
     ...
     * @return
     */
	public String getBcfbRequest(GpsHeader header,String xhs){
		Bcfb bcfb=new Bcfb();
	 ...
	 * @return
	 */
	public String getRMsg(GpsHeader header,byte[] b2,String msgid){
		//查看是否需要分包
		String hexstring="";
