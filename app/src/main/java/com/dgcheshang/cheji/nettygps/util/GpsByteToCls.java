...
public class GpsByteToCls {
	 ...
	 * @return
	 */
	public static GpsHeader getHeader(byte[] b){
		try{
			GpsHeader header=new GpsHeader();
			//消息ID
			byte[] temp= ByteUtil.subBytes(b, 0, 2);
			header.setMsgid(ByteUtil.bytesToHexString(temp));
			//消息体属性
			String s1= Integer.toBinaryString((b[2] & 0xFF) + 0x100).substring(1);
			String s2= Integer.toBinaryString((b[3] & 0xFF) + 0x100).substring(1);
			String s12=s1+s2;
			header.setBodyprop(s12);

			if(s12.substring(2,3).equals("1")){
				temp= ByteUtil.subBytes(b, 12, 2);
				header.setMsgpackcnt(ByteUtil.byte2ToUnsignedShort(temp, 0));
				temp= ByteUtil.subBytes(b, 14, 2);
	 ...
	 * @return
	 */
	public static GpsZdzcR getZdzcR(byte[] sbody){
		GpsZdzcR zr=new GpsZdzcR();
		byte[] temp= ByteUtil.subBytes(sbody, 0, 2);
		...
		if(zr.getJg()==0){
			temp= ByteUtil.subBytes(sbody,3,sbody.length-3);
			String hs= ByteUtil.clearHexStringback(ByteUtil.bytesToHexString(temp));
			zr.setAuthCode(new String(ByteUtil.hexStringToByte(hs)));
		}
		return zr;
	}
	...
	public static Object getObject(String msgid,byte[] sbody){
		 if(msgid.equals("8101")){
			//教练员登陆应答
			return GpsByteToCls.getJlydlR(sbody);
		}else if(msgid.equals("8102")){
			//教练员登出应答
			return GpsByteToCls.getJlydcR(sbody);
		}else if(msgid.equals("8201")){
			//学员登录应答
			return GpsByteToCls.getXydlR(sbody);
		}else if(msgid.equals("8202")){
			//学员登出应答
			return GpsByteToCls.getXydcR(sbody);
		}else if(msgid.equals("8205")){
			//命令上报学时记录
			return GpsByteToCls.getCxxsjl(sbody);
		}else if(msgid.equals("0205")){
			//命令上报学时记录应答
			return GpsByteToCls.getCxxsjlR(sbody);
		}else if(msgid.equals("8301")){
			//立即拍照
			return GpsByteToCls.getLjpz(sbody);
		}else if(msgid.equals("0301")){
			//立即拍照应答
			return GpsByteToCls.getLjpzR(sbody);
		}else if(msgid.equals("8302")){
			//查询照片
			return GpsByteToCls.getCxzp(sbody);
		}else if(msgid.equals("0302")){
			//查询照片应答
			return GpsByteToCls.getCxzpR(sbody);
		}else if(msgid.equals("0303")){
			//查询照片应答
			return GpsByteToCls.getSbzpjg(sbody);
		}else if(msgid.equals("8303")){
			//上报照片结果应答
			return GpsByteToCls.getSbzpjgR(sbody);
		}else if(msgid.equals("8304")){
			//上传指定照片
			return GpsByteToCls.getZdzp(sbody);
		}else if(msgid.equals("0304")||msgid.equals("8305")){
			//上传指定照片应答
			return GpsByteToCls.getSczdzpR(sbody);
		}else if(msgid.equals("8501")){
			//设置计时终端应用参数
			return GpsByteToCls.getYycs(sbody);
		}else if(msgid.equals("0501")){
			//设置应用参数回复
			return GpsByteToCls.getYycsR(sbody);
		}else if(msgid.equals("8502")){
			//设置禁训状态
			return GpsByteToCls.getJxzt(sbody);
		}else if(msgid.equals("0502")){
			//设置禁训状态回复
			return GpsByteToCls.getJxztR(sbody);
		}else if(msgid.equals("0503")){
			//查询应用参数回复
			return GpsByteToCls.getCxyycsR(sbody);
		}else if(msgid.equals("0401")){
			//请求身份认证
			return GpsByteToCls.getSfrz(sbody);
		}else if(msgid.equals("8401")){
			//请求身份认证回复
			return GpsByteToCls.getSfrzR(sbody);
		}else if(msgid.equals("0402")){
			//请求统一编号
			return GpsByteToCls.getQqtybh(sbody);
		}else if(msgid.equals("8402")){
			//请求统一编号应答
			return GpsByteToCls.getQqtybhR(sbody);
		}else if(msgid.equals("7001")){
			return GpsByteToCls.getImeiPasswordR(sbody);
		 }
		return null;
	}
